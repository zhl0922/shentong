<template>
    ...
    <div class="page page-rank" v-if="!$loadingRouteData">
        <div class="rank-list pt20">
            ...
            <table v-show="status==2" v-lazy="500">
                <thead class="fs-white fs-28">
                    <tr>
                        <th><span>排名</span></th>
                        <th width="45%"><span>姓名</span></th>
            ...
            <table v-show="status==3" v-lazy="500">
                <thead class="fs-white fs-28">
                    <tr>
                        <th><span>排名</span></th>
...
<script>
    ...
    export default {
        ...
        methods: {
            handleTab: async function(status, list) {
                this.status = status;
                if(list.length) return;
                switch(status) {
                    case 2:
                        await this.$xhr.getUserInfo();
                        this.$xhr.getData('rank/getRank', {
                            sort: 2
                        })
                        .then((list) => {
                    ...
                    case 3: 
                        await this.$xhr.getUserInfo();
                        this.$xhr.getData('rank/getDepartmentRank')
                        .then((list) => {
        ...
        route: {
            data: async function() {
                await this.$xhr.getUserInfo();
                return this.$xhr.getData('rank/getRank', {
                    sort: 1
                })
                .then((list) => {
